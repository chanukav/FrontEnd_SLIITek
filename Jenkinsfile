        ...
        stage('Deploy to EC2 via SSH') {
            ...
            steps {
                withCredentials([sshUserPrivateKey(
                    ...
                    bat 'ssh -o StrictHostKeyChecking=no -i "%SSH_KEY_PATH%" %SSH_USER%@%EC2_HOST% "PREV_TAG=$(grep -E \'^FRONTEND_IMAGE_TAG=\' /home/%SSH_USER%/.env | cut -d\'=\' -f2) && PREV_TAG=${PREV_TAG:-latest} && touch /home/%SSH_USER%/.env && sed -i \'/^FRONTEND_IMAGE_TAG=/d\' /home/%SSH_USER%/.env && echo \'FRONTEND_IMAGE_TAG=%IMAGE_TAG%\' >> /home/%SSH_USER%/.env && export AWS_ACCOUNT_ID=%AWS_ACCOUNT_ID% && export AWS_REGION=%AWS_DEFAULT_REGION% && aws ecr get-login-password --region %AWS_DEFAULT_REGION% | docker login --username AWS --password-stdin %AWS_ACCOUNT_ID%.dkr.ecr.%AWS_DEFAULT_REGION%.amazonaws.com && docker compose -f /home/%SSH_USER%/docker-compose.yml pull frontend && docker compose -f /home/%SSH_USER%/docker-compose.yml up -d --no-deps frontend && echo \'Performing health check...\' && success=0 && for i in 1 2 3 4 5 6; do if curl -f http://localhost:3000/api/health; then success=1; break; fi; sleep 5; done && if [ $success -eq 0 ]; then echo \'Health check failed! Rolling back...\' && sed -i \'/^FRONTEND_IMAGE_TAG=/d\' /home/%SSH_USER%/.env && echo \\\"FRONTEND_IMAGE_TAG=$PREV_TAG\\\" >> /home/%SSH_USER%/.env && docker compose -f /home/%SSH_USER%/docker-compose.yml pull frontend && docker compose -f /home/%SSH_USER%/docker-compose.yml up -d --no-deps frontend && exit 1; fi && docker image prune -f"'
                }
            }
        }
    }
