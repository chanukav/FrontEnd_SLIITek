        ...
        stage('Deploy to EC2 via SSH') {
            steps {
                withCredentials([sshUserPrivateKey(
                    ...
                    // Connect to EC2, authenticate with ECR, pull the new frontend image, and recreate the service
                    bat 'ssh -o StrictHostKeyChecking=no -i "%SSH_KEY_PATH%" %SSH_USER%@%EC2_HOST% "export AWS_ACCOUNT_ID=%AWS_ACCOUNT_ID% && export AWS_REGION=%AWS_DEFAULT_REGION% && aws ecr get-login-password --region %AWS_DEFAULT_REGION% | docker login --username AWS --password-stdin %ECR_REGISTRY% && docker compose -f /home/%SSH_USER%/docker-compose.yml pull frontend && docker compose -f /home/%SSH_USER%/docker-compose.yml up -d frontend && docker image prune -f"'
                }
            }
        }
    }
